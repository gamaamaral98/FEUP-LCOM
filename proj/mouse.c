...
int (mouse_enable_disable_data)(u32_t command) //DISABLE ENABLE
{
    ...
    while (i<5) {
        ...
        if ((status & IBF) !=0) {
            printf("IBF not ready!");
            continue;
        } //Input buffer not ready and not reading from keyboard or mouse
...
void (packet_parser)(struct packet *mouse_packet, uint8_t scan_codes[]){
    ...
    if(scan_codes[0] & MSB_X_DELTA) mouse_packet->delta_x = scan_codes[1] | 0xFF00;
    else mouse_packet->delta_x = scan_codes[1] & 0x00FF;

    if(scan_codes[0] & MSB_Y_DELTA) mouse_packet->delta_y = scan_codes[2] | 0xFF00;
    else mouse_packet->delta_y = scan_codes[2] & 0x00FF;
...
void (mouse_ih)() {
    ...
    if( status & OBF ) {
        ...
        if(sys_inb(OUT_BUF, &value_mouse) == 0){
